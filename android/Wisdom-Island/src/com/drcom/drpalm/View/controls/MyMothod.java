...
public class MyMothod {
	...
	public static int Dp2Px(Context context, int dpvalue) {
		int px = (int) (dpvalue
				* (double) context.getResources().getDisplayMetrics().densityDpi / 160);
		return px;
	}
	...
	public static int Px2Dp(Context context, int pxvalue) {
		int dp = (int) (pxvalue / ((double) context.getResources()
				.getDisplayMetrics().densityDpi / 160));
		return dp;
	}
	 ...
	 * @return
	 */
	public static Bitmap Byte2Bitmap(byte[] b,boolean compress) {
		try {
			if (b.length != 0) {
				// 如果我们把它设为true，
				// 那么BitmapFactory.decodeFile(String path, Options
				...
				// 计算缩放比
				if(compress){
					int nMax = options.outHeight > options.outWidth ? options.outHeight
							: options.outWidth;
				...
				Bitmap backb = BitmapFactory.decodeByteArray(b, 0, b.length,
						options);
				bitmap = null;
				b = null;
				return backb;
	...
	/**
	 * 合成图片 create the bitmap from a byte array
	 * 
	 * @param src
	...
	public static Bitmap mixBitmap(Bitmap src, Bitmap mark, int x, int y) {
		...
		try {
			...
			newb = Bitmap.createBitmap(w, h, Config.ARGB_8888);// 内存不足，则创建一个新的和SRC长度宽度
																// 2/3 的位图
	 ...
	 * @return
	 */
	public static byte[] zoomSourceByte(int des_width, int des_height,
			byte[] source) {
		Bitmap oldBitmp = MyMothod.Byte2Bitmap(source,false);
		...
		Bitmap newBitmap = Bitmap.createBitmap(oldBitmp, 0, 0, width, height,
				matrix, true);
		// if(!oldBitmp.isRecycled()){
		// oldBitmp.recycle();
		oldBitmp = null;
		// }
		return getBitmapByte(Bitmap.createBitmap(newBitmap, 0, 0, des_width,
				des_height));
	}
	...
	public static byte[] compressFile(String strpath) {
		...
		try {
			...
			Toast.makeText(GlobalVariables.gAppContext,
					"图片太大了", Toast.LENGTH_SHORT)
					.show();
			e.printStackTrace();
		}
		...
		try {
			ByteArrayOutputStream out = new ByteArrayOutputStream();
			if (bitmap.compress(Bitmap.CompressFormat.JPEG, CompressQuality,
					out)) {
				bitmap.recycle();
	 ...
	 * 根据字段获取图片资源id
	 **/
	public static Bitmap getIcon(Context mContext, String iconName) {
		Bitmap bm;
		int id = mContext.getResources().getIdentifier(iconName, "drawable",
				mContext.getApplicationInfo().packageName);
		if(id == 0){
			bm = new ResourceManagement().getBitmapFromFile(mContext.getResources().getString(R.string.mainitems_icons_path)+iconName+".png");
		}else{
	 ...
	 * 根据字段获取字符串资源
	 **/
	public static String getStringFromRes(Context mContext, String strName) {
		String str;
		int id = mContext.getResources().getIdentifier(strName, "string",
				mContext.getApplicationInfo().packageName);
		if (id != 0) {
